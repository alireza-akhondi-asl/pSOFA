...
#' Neurologic function that calculates a score based on GCS data
#'
...
#' @param clinical_events A character string vector representing the name of the clinical events corresponding to GCS (default value is "GCS")
#' @return An integer between 0 and 4
#' @examples
#'
Neurologic <- function(DT, V, E, day_of_interest = 0, clinical_events = c("GCS")) {
  ...
  # Calculate the neurologic score based on the values of the filtered V vector
  score <- 5-cut(min(V),breaks=c(-Inf,6,10,13,15,Inf),labels=FALSE,right=FALSE)  
